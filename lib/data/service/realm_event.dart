...
class RealmEvent {
  ...
  static Realm realm = Realm(Configuration.local(
      ...
      isReadOnly: false, schemaVersion: 1));
  ...
  //
  //   realm.write(
  //       () => {realm.deleteAll<ProductUnits>(), realm.deleteAll<Products>()});
  //
  //   // realm.close();
  //   // if(realm.isClosed) {
  ...
  //     }
  //     runUnit += 1;
  //     var result = await syncProduct(StreamEvent.siteId, skus.join("|"));
  //     if (result.isEmpty) {
  //       break;
  //     }
  ...
  // static Future<List<String>> syncProduct(String siteId, String skus) async {
  //   List<String> resp = [];
  //   var result = await HttpRealmService.getProducts(siteId, skus, "");
